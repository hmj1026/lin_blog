...
export function PostCard({ post, layout = "vertical" }: PostCardProps) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className={`group rounded-2xl border border-line bg-white p-5 shadow-card transition hover:-translate-y-1 hover:shadow-soft ${
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <h3 className="font-display text-xl leading-snug text-primary transition group-hover:text-accent-600">
          {post.title}
        </h3>
        <p className="text-sm text-base-300">{post.excerpt}</p>
        <div className="flex flex-wrap gap-2">
          {post.tags.map((tag) => (
            <span key={tag} className="rounded-full bg-base-100 px-3 py-1 text-xs font-semibold text-primary">
              #{tag}
            </span>
          ))}
        </div>
