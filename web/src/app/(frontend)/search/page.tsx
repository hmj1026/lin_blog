    ...
    <div className="section-shell space-y-8 py-12">
      ...
      <form action="/search" method="GET" className="flex gap-3">
        <input
          ...
          defaultValue={query}
          placeholder="輸入關鍵字搜尋..."
          className="flex-1 rounded-xl border border-line bg-white px-4 py-3 text-sm text-primary placeholder:text-base-300 focus:border-primary focus:outline-none"
        />
        <button
          type="submit"
      ...
      {results.length > 0 ? (
        ...
        <div className="rounded-2xl border border-line bg-white p-8 text-center">
          <p className="text-base-300">輸入關鍵字開始搜尋</p>
        </div>
      )}
    </div>
  );
