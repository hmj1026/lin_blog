    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-line bg-white p-6 shadow-card space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <DaysFilter baseUrl={`/admin/analytics/posts/${postId}`} currentDays={safeDays} />
          <div className="text-sm text-base-300">
...
function PagerLink({ href, label, disabled }: { href: string; label: string; disabled: boolean }) {
  if (disabled) {
    return (
      <span className={buttonStyles({ variant: "outline", size: "sm", className: "opacity-50 cursor-not-allowed" })}>
        {label}
      </span>
    );
  }
  return (
    <Link href={href as never} className={buttonStyles({ variant: "outline", size: "sm" })}>
      {label}
    </Link>
  );
}
